    ...
    <div class="mx-4 min-h-screen max-w-screen-xl sm:mx-8 xl:mx-auto">
        ...
        <div class="grid grid-cols-8 pt-3 sm:grid-cols-10">
            <div class="relative my-4 w-56 sm:hidden">
                ...
                <label for="select-1"
                    class="flex w-full cursor-pointer select-none rounded-lg border p-2 px-3 text-sm text-gray-700 ring-pinky peer-checked:ring">Profile
                </label>
                ...
                <ul
                    ...
                    <li class="cursor-pointer px-3 py-2 text-sm text-slate-600 hover:bg-pinky hover:text-white">social
                    </li>
                    <li class="cursor-pointer px-3 py-2 text-sm text-slate-600 hover:bg-pinky hover:text-white">Profile
                    </li>
                    <li class="cursor-pointer px-3 py-2 text-sm text-slate-600 hover:bg-pinky hover:text-white">Settings
                    </li>
                </ul>
            </div>
            ...
            <div class="col-span-2 hidden sm:block">
                <ul>
                    <li class="mt-5 cursor-pointer border-l-2 border-transparent px-2 py-2 font-semibold transition hover:border-l-pinky hover:text-pinky"
                        @click="changeView('social')">Social</li>
                    <li class="mt-5 cursor-pointer border-l-2 border-l-pinky px-2 py-2 font-semibold text-pinky transition hover:border-l-pinky hover:text-pinky"
                        @click="changeView('profile')">Profile</li>
                    <li class="mt-5 cursor-pointer border-l-2 border-transparent px-2 py-2 font-semibold transition hover:border-l-pinky hover:text-pinky"
                        @click="changeView('settings')">Settings</li>
                </ul>
            </div>

            <div class="col-span-8 overflow-hidden rounded-xl sm:bg-gray-50 sm:px-8 sm:shadow">
                <div class="pt-4">
                    <h1 class="py-2 text-2xl font-semibold">Profile Info</h1>
                    <!-- <p class="font- text-slate-600">Lorem ipsum dolor, sit amet consectetur adipisicing elit.</p> -->
                </div>
                <hr class="mt-4 mb-8" />
                <p class="py-2 text-xl font-semibold">Email Address</p>
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-gray-600">Your email address is <strong>{{ this.user.email }}</strong></p>
                    <button
                ...
                <p class="mt-2">Can't remember your current password. <a
                        class="text-sm font-semibold text-blue-600 underline decoration-2" href="#">Recover Account</a>
                </p>
                <input class="mt-4 rounded-lg bg-blue-600 px-4 py-2 text-white" type="submit" value="Save Password">
                </form>
                <hr class="mt-4 mb-8" />
                ...
                <div class="mb-10">
                    ...
                    <p class="mt-2">Make sure you have taken backup of your account in case you ever need to get access
                        ...
                        this action.</p>
                    <button class="ml-auto text-sm font-semibold text-rose-600 underline decoration-2">Continue with
                        deletion</button>
                </div>
            </div>
        </div>
...
export default {
  ...
  mounted () {
    console.log('profileinfovue', this.user)
  },
  methods: {
    changeView (view) {
