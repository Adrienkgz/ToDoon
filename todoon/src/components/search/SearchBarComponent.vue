<template>
  <div class="search-wrapper" :class="{ active: isActive }">
    <div class="input-holder">
      <input type="text" class="search-input" placeholder="Type to search" v-model="searchText" />
      <button class="search-icon" @click="toggleSearch"><span></span></button>
    </div>
    <span class="close" @click="toggleSearch"></span>
  </div>
</template>
...
.search-wrapper.active .input-holder .search-icon {
  ...
  margin: 10px;
  border-radius: 50px;
  transition: background-color 0.30s ease-in 0.38s;
